...
Now the advantage you get from this hierarchical structure provided by Moka should be clear: you place all the page-specific elements in the appropriate page directory, the elements that are used by multiple pages in the same group in the group directory, and all the elements that are used all over the site in the site directory. Moka always assembles your pages choosing the most specific version of an element (being it a layout, partial or variable). This means that you never have to write duplicated code.
...
Try typing in the command line something like:
  ...
  # and other_variable (with value "Moka Rocks!") in the project/site/my_group/my_page/variables.yml file
